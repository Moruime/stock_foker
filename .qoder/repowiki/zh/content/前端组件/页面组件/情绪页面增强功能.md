# 情绪页面增强功能

<cite>
**本文档引用的文件**
- [SentimentPage.tsx](file://frontend/src/pages/SentimentPage.tsx)
- [sentiment_agent.py](file://backend/app/agents/sentiment_agent.py)
- [agent_router.py](file://backend/app/routers/agent_router.py)
- [prompts.py](file://backend/app/llm/prompts.py)
- [useDataSource.ts](file://frontend/src/hooks/useDataSource.ts)
- [api.ts](file://frontend/src/services/api.ts)
- [data_source_service.py](file://backend/app/services/data_source_service.py)
- [data_fetcher.py](file://backend/app/services/data_fetcher.py)
- [base_agent.py](file://backend/app/agents/base_agent.py)
- [models.py](file://backend/app/models/models.py)
- [schemas.py](file://backend/app/models/schemas.py)
- [sector_agent.py](file://backend/app/agents/sector_agent.py)
- [SnapshotPanel.tsx](file://frontend/src/components/SnapshotPanel.tsx)
- [README.md](file://README.md)
- [产品设计文档.md](file://doc/产品设计文档.md)
- [2026-04-15-agent-perf-optimization.md](file://doc/迭代记录/2026-04-15-agent-perf-optimization.md)
</cite>

## 更新摘要
**变更内容**
- 更新了Sentiment Agent的并行数据获取架构
- 新增了三级缓存降级策略和问财API熔断机制
- 重构了数据源缓存管理和服务层架构
- 增强了系统性能优化和故障处理能力

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构概览](#架构概览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)

## 简介

Stock Foker 是一款个人A股分析辅助应用，专注于消息面情绪分析功能。该项目集成了K线行情、技术指标、AI多维度分析与交易记录管理，特别强化了消息面情绪分析能力，通过LLM对个股新闻进行深度分析，提供可解释性的投资决策支持。

消息面情绪分析是该项目的核心特色功能，它能够：
- 自动抓取和分析个股相关新闻、公告、研报
- 识别有效信息与噪音，提供多空情绪评分
- 生成结构化的分析报告和投资建议
- 支持实时刷新和历史数据回溯

**更新** 系统经过重大性能优化，Sentiment Agent从串行数据获取改为并行获取，显著提升了响应速度和用户体验。

## 项目结构

该项目采用前后端分离的架构设计，主要分为以下几个核心模块：

```mermaid
graph TB
subgraph "前端层 (React)"
A[SentimentPage.tsx<br/>消息面情绪分析页面]
B[SnapshotPanel.tsx<br/>Agent历史快照面板]
C[useDataSource.ts<br/>数据源Hook]
D[api.ts<br/>API服务封装]
end
subgraph "后端层 (FastAPI)"
E[agent_router.py<br/>Agent路由]
F[sentiment_agent.py<br/>消息面Agent]
G[data_source_service.py<br/>数据源服务]
H[prompts.py<br/>LLM提示模板]
I[data_fetcher.py<br/>数据获取服务]
end
subgraph "数据层"
J[models.py<br/>数据库模型]
K[schemas.py<br/>数据验证模型]
end
A --> D
B --> D
C --> D
D --> E
E --> F
E --> G
F --> H
F --> I
E --> J
E --> K
G --> I
```

**图表来源**
- [SentimentPage.tsx:1-577](file://frontend/src/pages/SentimentPage.tsx#L1-L577)
- [agent_router.py:1-395](file://backend/app/routers/agent_router.py#L1-L395)
- [sentiment_agent.py:1-99](file://backend/app/agents/sentiment_agent.py#L1-L99)
- [data_source_service.py:1-257](file://backend/app/services/data_source_service.py#L1-L257)

**章节来源**
- [README.md:1-128](file://README.md#L1-L128)
- [产品设计文档.md:1-446](file://doc/产品设计文档.md#L1-L446)

## 核心组件

### 消息面情绪分析Agent

消息面情绪分析Agent是整个系统的核心组件，负责整合多源数据并进行AI分析：

```mermaid
classDiagram
class BaseAgent {
+execute() AgentResult
+fetch_data() dict
+build_prompt() list
+parse_response() dict
+fallback() dict
}
class SentimentAgent {
+agent_name : "sentiment"
+fetch_data() dict
+build_prompt() list
+parse_response() dict
+fallback() dict
}
class AgentResult {
+agent_name : string
+status : string
+data : dict
+llm_used : boolean
+timestamp : string
+error_message : string
+to_dict() dict
}
BaseAgent <|-- SentimentAgent
SentimentAgent --> AgentResult : "返回"
```

**图表来源**
- [base_agent.py:1-119](file://backend/app/agents/base_agent.py#L1-L119)
- [sentiment_agent.py:1-99](file://backend/app/agents/sentiment_agent.py#L1-L99)

### 数据源管理系统

系统实现了多层次的数据缓存机制，确保数据获取的高效性和可靠性：

```mermaid
flowchart TD
A[前端请求) --> B{检查Agent缓存}
B --> |命中| C[返回缓存数据]
B --> |未命中| D[调用后端API]
D --> E[检查Agent结果缓存]
E --> |命中| F[返回缓存结果]
E --> |未命中| G[执行Agent分析]
G --> H[并行获取多源数据]
H --> I[三级缓存降级策略]
I --> J[问财API熔断机制]
J --> K[LLM情绪分析]
K --> L[保存Agent缓存]
L --> M[返回分析结果]
subgraph "数据源层次"
N[Agent缓存]
O[数据源缓存]
P[原始API数据]
end
```

**图表来源**
- [agent_router.py:186-201](file://backend/app/routers/agent_router.py#L186-L201)
- [data_source_service.py:84-257](file://backend/app/services/data_source_service.py#L84-L257)

**章节来源**
- [sentiment_agent.py:15-80](file://backend/app/agents/sentiment_agent.py#L15-L80)
- [data_source_service.py:1-257](file://backend/app/services/data_source_service.py#L1-L257)

## 架构概览

消息面情绪分析系统的整体架构采用了分层设计，确保了系统的可扩展性和可维护性：

```mermaid
graph TB
subgraph "用户界面层"
UI[React页面组件]
Hooks[自定义Hooks]
end
subgraph "API服务层"
API[HTTP API封装]
Router[FastAPI路由]
end
subgraph "业务逻辑层"
Agent[Agent执行器]
Prompt[LLM提示模板]
Cache[缓存管理]
Parallel[并行数据获取]
Circuit[熔断机制]
end
subgraph "数据访问层"
DS[数据源服务]
DF[数据获取服务]
DB[(SQLite数据库)]
end
subgraph "外部数据源"
News[新闻API]
Ann[公告API]
Reports[研报API]
Basic[基本资料API]
end
UI --> API
Hooks --> API
API --> Router
Router --> Agent
Agent --> Prompt
Agent --> Cache
Agent --> Parallel
Parallel --> DS
Parallel --> DF
Cache --> Circuit
Cache --> DB
DS --> News
DS --> Ann
DS --> Reports
DS --> Basic
DS --> DB
```

**图表来源**
- [api.ts:117-154](file://frontend/src/services/api.ts#L117-L154)
- [agent_router.py:186-201](file://backend/app/routers/agent_router.py#L186-L201)
- [prompts.py:12-106](file://backend/app/llm/prompts.py#L12-L106)

## 详细组件分析

### 前端消息面页面组件

消息面情绪分析页面是用户交互的核心界面，提供了丰富的数据展示和交互功能：

```mermaid
sequenceDiagram
participant User as 用户
participant Page as SentimentPage
participant Hook as useDataSource
participant API as API服务
participant Agent as Agent服务
User->>Page : 访问消息面页面
Page->>Page : 检查Agent缓存
alt 缓存命中
Page->>Page : 显示缓存数据
else 缓存未命中
Page->>API : 调用/run-sentiment
API->>Agent : 执行消息面分析
Agent->>Agent : 并行获取多源数据
Agent->>Agent : 三级缓存降级策略
Agent->>Agent : 问财API熔断机制
Agent->>Agent : LLM情绪分析
Agent-->>API : 返回分析结果
API-->>Page : 返回结果
Page->>Page : 更新UI显示
end
User->>Page : 点击刷新按钮
Page->>API : 清除缓存并重新获取
Page->>Hook : 触发数据源刷新
Hook->>API : 刷新数据源
```

**图表来源**
- [SentimentPage.tsx:89-134](file://frontend/src/pages/SentimentPage.tsx#L89-L134)
- [api.ts:117-120](file://frontend/src/services/api.ts#L117-L120)

页面的主要功能包括：

1. **实时数据展示**：显示当前股票的消息面情绪分析结果
2. **多源数据整合**：整合新闻、公告、研报、公司资料等多源数据
3. **交互式刷新**：支持一键刷新和独立数据源刷新
4. **历史数据回溯**：通过快照面板查看历史分析结果

**章节来源**
- [SentimentPage.tsx:70-577](file://frontend/src/pages/SentimentPage.tsx#L70-L577)

### Agent执行流程

消息面Agent的执行流程体现了完整的AI分析管道：

```mermaid
flowchart TD
A[接收请求] --> B[检查Agent缓存]
B --> |命中| C[直接返回缓存]
B --> |未命中| D[并行获取数据]
D --> E[获取新闻数据]
D --> F[获取事件数据]
D --> G[获取资讯数据]
D --> H[获取公告数据]
D --> I[获取研报数据]
D --> J[获取公司资料]
D --> K[获取业务数据]
D --> L[获取股东数据]
E --> M[三级缓存降级策略]
F --> M
G --> M
H --> M
I --> M
J --> M
K --> M
L --> M
M --> N[问财API熔断机制]
N --> O[LLM情绪分析]
O --> P[解析分析结果]
P --> Q[保存缓存]
Q --> R[返回结果]
C --> R
```

**图表来源**
- [sentiment_agent.py:15-80](file://backend/app/agents/sentiment_agent.py#L15-L80)
- [prompts.py:12-106](file://backend/app/llm/prompts.py#L12-L106)

**章节来源**
- [base_agent.py:62-102](file://backend/app/agents/base_agent.py#L62-L102)
- [agent_router.py:186-201](file://backend/app/routers/agent_router.py#L186-L201)

### 数据源缓存机制

系统实现了多层次的缓存机制，确保数据获取的高效性：

```mermaid
classDiagram
class DataSourceCache {
+id : Integer
+stock_code : String
+source_type : String
+cache_key : String
+data : Text
+created_at : DateTime
}
class AgentResultCache {
+id : Integer
+agent_name : String
+stock_code : String
+cache_key : String
+status : String
+llm_used : Integer
+data : Text
+error_message : Text
+created_at : DateTime
}
class DailyAgentSnapshot {
+id : Integer
+agent_type : String
+stock_code : String
+date : String
+snapshot_data : Text
+llm_used : Integer
+created_at : DateTime
+updated_at : DateTime
}
class useDataSource {
+data : Record
+loading : boolean
+error : string
+timestamp : string
+fromCache : boolean
+refresh() void
}
DataSourceCache <.. useDataSource : "被使用"
AgentResultCache <.. useDataSource : "被使用"
DailyAgentSnapshot <.. useDataSource : "被使用"
```

**图表来源**
- [models.py:118-151](file://backend/app/models/models.py#L118-L151)
- [useDataSource.ts:14-21](file://frontend/src/hooks/useDataSource.ts#L14-L21)

**章节来源**
- [data_source_service.py:84-257](file://backend/app/services/data_source_service.py#L84-L257)
- [useDataSource.ts:1-169](file://frontend/src/hooks/useDataSource.ts#L1-L169)

## 依赖关系分析

系统各组件之间的依赖关系体现了清晰的分层架构：

```mermaid
graph TD
subgraph "前端依赖"
A[SentimentPage.tsx] --> B[api.ts]
A --> C[useDataSource.ts]
A --> D[SnapshotPanel.tsx]
B --> E[axios]
C --> F[React Hooks]
D --> B
end
subgraph "后端依赖"
G[agent_router.py] --> H[FastAPI]
G --> I[SQLAlchemy]
G --> J[SentimentAgent]
J --> K[BaseAgent]
J --> L[Prompts]
J --> M[DataFetcher]
J --> N[DataSourceService]
G --> O[models.py]
G --> P[schemas.py]
end
subgraph "数据依赖"
Q[DataSourceCache] --> R[SQLite]
S[AgentResultCache] --> R
T[DailyAgentSnapshot] --> R
end
A --> G
G --> Q
G --> S
G --> T
```

**图表来源**
- [api.ts:1-188](file://frontend/src/services/api.ts#L1-L188)
- [agent_router.py:1-395](file://backend/app/routers/agent_router.py#L1-L395)
- [models.py:1-151](file://backend/app/models/models.py#L1-L151)

**章节来源**
- [schemas.py:1-213](file://backend/app/models/schemas.py#L1-L213)
- [README.md:33-41](file://README.md#L33-L41)

## 性能考虑

系统在设计时充分考虑了性能优化，采用了多种策略来提升用户体验：

### 并行数据获取架构

**更新** 系统实现了全新的并行数据获取架构，显著提升了性能表现：

```mermaid
flowchart TD
A[Sentiment Agent请求] --> B[parallel_get_data_sources]
B --> C[ThreadPoolExecutor]
C --> D[8个数据源并行获取]
D --> E[stock_news]
D --> F[hithink_events]
D --> G[hithink_news]
D --> H[announcements]
D --> I[reports]
D --> J[basicinfo]
D --> K[business]
D --> L[shareholders]
E --> M[独立DB Session]
F --> M
G --> M
H --> M
I --> M
J --> M
K --> M
L --> M
M --> N[结果聚合]
N --> O[返回给Agent]
```

**图表来源**
- [sentiment_agent.py:27-64](file://backend/app/agents/sentiment_agent.py#L27-L64)
- [data_source_service.py:224-257](file://backend/app/services/data_source_service.py#L224-L257)

### 三级缓存降级策略

系统实现了智能的三级缓存降级策略，确保在各种情况下都能提供稳定的服务：

```mermaid
flowchart TD
A[数据请求] --> B{检查当日新鲜缓存}
B --> |命中且非空| C[返回新鲜缓存]
B --> |未命中| D[调用API]
D --> E{API返回有效数据}
E --> |是| F[写入缓存并返回]
E --> |否| G[检查历史缓存回退]
G --> H{找到有效历史缓存}
H --> |是| I[返回历史缓存]
H --> |否| J[返回空数据]
```

**图表来源**
- [data_source_service.py:163-213](file://backend/app/services/data_source_service.py#L163-L213)

### 问财API熔断机制

**新增** 系统实现了问财API熔断机制，防止API额度耗尽导致的性能问题：

```mermaid
flowchart TD
A[API调用请求] --> B{_iwencai_circuit_open}
B --> |True| C[立即返回空数据]
B --> |False| D[执行API调用]
D --> E{收到403响应}
E --> |是| F[_check_iwencai_403触发熔断]
F --> G[_iwencai_circuit_open = True]
G --> H[记录熔断原因]
H --> I[后续调用全部熔断]
E --> |否| J[正常返回数据]
```

**图表来源**
- [data_fetcher.py:31-66](file://backend/app/services/data_fetcher.py#L31-L66)

### 缓存策略
- **Agent结果缓存**：每日09:00边界缓存，避免重复LLM调用
- **数据源缓存**：独立的数据源缓存，支持强制刷新
- **前端内存缓存**：模块级内存缓存，跨组件共享
- **并行数据获取**：8个数据源统一并行获取，提升响应速度

### 数据优化
- **增量更新**：K线数据采用增量更新策略
- **数据压缩**：JSON序列化存储，减少数据库占用
- **熔断保护**：API调用失败时快速失败，避免长时间等待

**章节来源**
- [2026-04-15-agent-perf-optimization.md:1-63](file://doc/迭代记录/2026-04-15-agent-perf-optimization.md#L1-L63)

## 故障排除指南

### 常见问题及解决方案

1. **LLM配置问题**
   - 检查.env文件中的LLM配置
   - 确认API密钥和基础URL正确
   - 使用`/api/agent/reload-config`重新加载配置

2. **数据获取失败**
   - 检查网络连接和API密钥
   - 查看后端日志获取详细错误信息
   - 验证股票代码和名称的准确性
   - **新增** 检查问财API熔断状态：使用`/api/agent/iwencai-status`

3. **缓存问题**
   - 使用`/api/agent/cache/{stock_code}`清除缓存
   - 检查数据库连接状态
   - 验证缓存键的正确性
   - **新增** 检查三级缓存状态：确认当日缓存、历史缓存的有效性

4. **性能问题**
   - **新增** 检查并行数据获取状态
   - 验证ThreadPoolExecutor配置
   - 监控API调用频率和熔断状态

**章节来源**
- [agent_router.py:384-395](file://backend/app/routers/agent_router.py#L384-L395)
- [MVP实现说明.md:85-92](file://doc/MVP实现说明.md#L85-L92)

## 结论

Stock Foker的消息面情绪分析功能通过精心设计的架构和实现，为用户提供了一个强大而易用的投资决策辅助工具。经过重大性能优化后，系统的主要优势包括：

1. **并行数据获取**：8个数据源统一并行获取，显著提升响应速度
2. **智能缓存策略**：三级缓存降级策略确保服务稳定性
3. **熔断保护机制**：问财API熔断机制防止性能问题扩散
4. **完整的数据整合**：支持多源数据的统一管理和分析
5. **高效的缓存机制**：多层次缓存确保快速响应
6. **可解释的AI分析**：提供详细的分析过程和推理依据
7. **灵活的交互设计**：支持实时刷新和历史数据回溯
8. **可靠的错误处理**：完善的降级策略确保系统稳定性

该系统不仅满足了当前的功能需求，还为未来的扩展奠定了良好的基础。通过持续的优化和改进，相信能够为个人投资者提供更加精准和实用的投资决策支持。

**更新** 性能优化效果显著，Sentiment Agent从44秒优化至20.9秒，性能提升约53%，为用户提供了更加流畅的使用体验。