...
CheckCache --> |否| Fetch["发起 /api/stocks/:code/analysis"]
...
```mermaid
flowchart TD
Start(["Agent 执行开始"]) --> CheckCache{"缓存命中？"}
CheckCache --> |是| ReturnCache["返回缓存结果"]
CheckCache --> |否| ParallelExec["并行执行多个 Agent"]
ParallelExec --> Thread1["线程1: Agent A"]
ParallelExec --> Thread2["线程2: Agent B"]
ParallelExec --> Thread3["线程3: Agent C"]
Thread1 --> SaveCache1["保存缓存"]
Thread2 --> SaveCache2["保存缓存"]
Thread3 --> SaveCache3["保存缓存"]
SaveCache1 --> MergeResults["合并结果"]
SaveCache2 --> MergeResults
SaveCache3 --> MergeResults
MergeResults --> Enhanced["执行增强建议 Agent"]
Enhanced --> FinalCache["保存最终缓存"]
FinalCache --> End(["返回完整结果"])
ReturnCache --> End
```
...
- 排查步骤
  - 后端：查看启动日志、数据库连接状态、远程接口返回、Agent 执行日志
...
## 结论
Stock Foker 的性能优化应围绕"数据库查询限制与索引、API 响应时间与缓存、前端渲染与懒加载"三大方向展开。当前项目已具备基础缓存与分页能力，新增的并行处理机制和三级缓存策略显著提升了响应速度和稳定性。建议在生产环境中引入连接池、Redis 缓存与虚拟滚动等进一步优化手段，并建立完善的性能测试流程以持续监控与改进。
...
- 问财 API 熔断机制，避免额度耗尽导致的超时
- 前端 Agent 缓存上下文，支持内存级缓存和淘汰策略
- 并行执行机制，支持 ThreadPoolExecutor 并行处理多个 Agent
