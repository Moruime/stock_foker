...
## 更新摘要
**变更内容**
- 新增K线数据强制刷新功能，支持用户手动刷新缓存的K线数据
- 在路由层和API层新增refresh参数支持
- 更新缓存策略以支持强制刷新逻辑
- 保持向后兼容性，refresh参数默认为False
...
**更新** 新增K线数据强制刷新功能，允许用户在需要时手动刷新缓存的K线数据，满足实时市场数据需求。
...
## 架构总览
K线数据获取的端到端流程如下：前端发起请求 → 路由层解析参数（含强制刷新选项）→ 服务层先查本地缓存，再按策略拉取远程数据，写入缓存并返回；随后可选地计算技术指标与生成建议。
...
FE->>API : GET /api/stocks/{code}/kline?refresh=true
API->>SVC : get_kline_data(code, period, dates, db, force_refresh)
alt 强制刷新
SVC->>SVC : force_refresh=True
SVC->>SVC : 跳过缓存检查
SVC->>SINA : 直接拉取远程数据
SINA-->>SVC : K线数据(JSONP)
SVC->>DB : 增量写入/更新当日
...
API->>SVC : get_kline_data(code, period, dates, db, force_refresh=False)
...
- 强制刷新：当 force_refresh=True 时，跳过缓存检查，直接拉取远程数据。
...
HasCache --> |是| ForceRefresh{"force_refresh=True？"}
ForceRefresh --> |是| FetchRemote
ForceRefresh --> |否| GapCheck["计算缓存最后日期与今日间隔"]
...
- 强制刷新优化：强制刷新模式下跳过缓存检查，确保数据实时性，适用于需要最新数据的场景。
...
- 强制刷新无效：确认 refresh 参数是否正确传递；检查 force_refresh 逻辑是否被正确执行。
- 指标为空：确认输入数据长度是否满足指标计算阈值；检查 NaN 值处理逻辑。
- 路由异常：核对路由参数（period、start_date、end_date、refresh）是否传入；查看 HTTP 异常码与错误信息。
...
## 结论
该 K线数据获取模块通过"本地缓存优先 + 多数据源降级"的策略，在保证数据可用性的前提下兼顾性能与稳定性。新增的强制刷新功能进一步增强了系统的灵活性，满足用户对实时数据的需求。标准化的数据格式与清晰的周期映射为后续技术指标计算与建议生成打下坚实基础。建议在生产环境中进一步完善监控与告警，针对不同数据源的可用性与延迟进行动态调整，并对大体量数据场景进行分页与缓存优化。

## 附录

### 接口定义与调用路径
- 获取 K 线数据：GET /api/stocks/{stock_code}/kline?refresh=false
- 获取完整分析：GET /api/stocks/{stock_code}/analysis?refresh=false

**更新** 新增 refresh 参数，支持强制刷新缓存的K线数据。默认值为 false，保持向后兼容性。
...
### 强制刷新功能详解
- 参数位置：在 /api/stocks/{stock_code}/kline 和 /api/stocks/{stock_code}/analysis 接口中
- 参数名称：refresh，布尔类型，默认值为 False
- 功能效果：当 refresh=True 时，跳过缓存检查，直接从远程数据源获取最新数据
- 使用场景：需要获取最新实时数据时，如盘中数据更新、紧急决策参考等
- 兼容性：默认 refresh=False，保持现有行为不变

**章节来源**
- [backend/app/routers/stock_router.py:90-141](file://backend/app/routers/stock_router.py#L90-L141)
- [backend/app/services/stock_service.py:221-342](file://backend/app/services/stock_service.py#L221-L342)