...
## 更新摘要
**变更内容**
- 新增浏览器缓存控制机制，确保 API 响应不被缓存
- 后端添加 NoCacheMiddleware 中间件，强制设置 Cache-Control 和 Pragma 头
- 前端 axios 配置中添加缓存控制请求头
...
- 主题定制、响应式与移动端适配
- 前端组件封装与状态管理最佳实践
- **新增** 浏览器缓存控制机制，确保数据新鲜度

## 项目结构
前端采用 React + Vite + Ant Design + ECharts 的组合；后端使用 FastAPI + SQLAlchemy + pandas_ta 实现技术指标计算与买卖建议生成。前端通过代理访问后端 API，后端提供统一的分析接口，返回 K 线、技术指标与买卖建议。**新增** NoCacheMiddleware 中间件确保 API 响应不被浏览器缓存。
...
- API 服务封装：api.ts 提供 getStockAnalysis 等方法，统一请求路径与参数传递，**新增**设置缓存控制请求头。
- 前端布局与交互：MainLayout 提供搜索、关注切换、时间框架选择等交互入口，AnalysisPage 基于上下文 focus 渲染图表。
- **新增** 缓存控制中间件：后端 NoCacheMiddleware 强制设置响应头，确保浏览器不会缓存 API 响应。
...
## 架构总览
前端通过 AnalysisPage 发起分析请求，后端在 stock_router 中整合 K 线查询、指标计算与建议生成，最终返回包含 K 线、指标与建议的结构化数据。前端据此渲染 ECharts 图表。**新增** NoCacheMiddleware 确保每次请求都获取最新数据。
...
- API 封装：api.ts 统一封装请求方法，便于测试与替换实现，**新增**设置缓存控制请求头。
...
## 结论
该图表展示集成以 React + ECharts 为核心，配合后端 pandas_ta 技术指标计算与建议生成，实现了从数据获取、指标计算到可视化呈现的完整链路。当前已具备 K 线、均线与成交量的叠加显示及基础交互能力；后续可在前端扩展 MACD/KDJ/布林带叠加与区域填充，在后端引入更丰富的指标与预测模型，并在前端引入虚拟滚动与移动端优化，以进一步提升性能与用户体验。**新增**的浏览器缓存控制机制确保了数据的新鲜度，避免了缓存导致的数据显示问题。
