...
## 更新摘要
**变更内容**
- 新增K线图表刷新按钮功能，用户可通过点击强制获取最新数据
- 前端AnalysisPage.tsx中新增handleKlineRefresh函数和klineRefreshing状态管理
- 后端stock_router.py和stock_service.py支持refresh参数，实现强制刷新机制
- 前端API服务新增refresh参数支持，后端getStockAnalysis接口接受force参数
- 增强数据新鲜度保证，避免图表显示过期数据
...
## 项目结构
前端采用 React + Vite + Ant Design + ECharts 的组合；后端使用 FastAPI + SQLAlchemy + pandas_ta 实现技术指标计算与买卖建议生成。前端通过代理访问后端 API，后端提供统一的分析接口，返回 K 线、技术指标与买卖建议。**新增** K线图表刷新功能增强数据实时性。

```mermaid
graph TB
subgraph "前端"
A["MainLayout.tsx<br/>布局与搜索/关注切换"]
B["AnalysisPage.tsx<br/>图表与分析页面<br/>新增刷新按钮"]
C["api.ts<br/>Axios 封装与路由<br/>缓存控制头"]
D["vite.config.ts<br/>开发服务器与代理"]
end
subgraph "后端"
E["main.py<br/>FastAPI 应用与CORS<br/>NoCacheMiddleware"]
F["stock_router.py<br/>分析接口<br/>支持refresh参数"]
G["stock_service.py<br/>K线与指标计算<br/>force_refresh支持"]
...
- [api.ts:18-21](file://frontend/src/services/api.ts#L18-L21)
- [vite.config.ts:1-16](file://frontend/vite.config.ts#L1-L16)
- [main.py:68-77](file://backend/app/main.py#L68-L77)
...
- [vite.config.ts:1-16](file://frontend/vite.config.ts#L1-L16)
- [main.py:1-88](file://backend/app/main.py#L1-L88)

## 核心组件
- 前端图表容器与配置：AnalysisPage 中的 ECharts 选项与 series 定义，支持 K 线、均线、成交量叠加显示，并内置 dataZoom 缩放与滑块。
- **新增** K线刷新按钮：AnalysisPage 中新增handleKlineRefresh函数和klineRefreshing状态，支持强制刷新当日K线数据。
...
- API 服务封装：api.ts 提供 getStockAnalysis 等方法，统一请求路径与参数传递，**新增**支持refresh参数。
- 前端布局与交互：MainLayout 提供搜索、关注切换、时间框架选择等交互入口，AnalysisPage 基于上下文 focus 渲染图表。
- 缓存控制中间件：后端 NoCacheMiddleware 强制设置响应头，确保浏览器不会缓存 API 响应。

**章节来源**
- [AnalysisPage.tsx:28-231](file://frontend/src/pages/AnalysisPage.tsx#L28-L231)
- [AnalysisPage.tsx:108-125](file://frontend/src/pages/AnalysisPage.tsx#L108-L125)
...
## 架构总览
前端通过 AnalysisPage 发起分析请求，后端在 stock_router 中整合 K 线查询、指标计算与建议生成，最终返回包含 K 线、指标与建议的结构化数据。前端据此渲染 ECharts 图表。**新增** K线刷新功能允许用户强制获取最新数据，**新增** NoCacheMiddleware 确保每次请求都获取最新数据。
...
participant M as "NoCacheMiddleware"
participant R as "stock_router.py"
participant S as "stock_service.py"
participant H as "advice_service.py"
U->>L : 选择股票/切换关注
L-->>P : 传入 focus 上下文
P->>A : getStockAnalysis(code, period)<br/>设置缓存控制头
A->>M : 发送请求<br/>强制禁用缓存
M-->>A : 设置 Cache-Control/Pragma 响应头
...
P-->>U : 渲染 K 线与指标图表
Note over U,P : 用户点击刷新按钮
U->>P : 点击刷新按钮
P->>P : handleKlineRefresh()
P->>A : getStockAnalysis(code, period, refresh=true)
A->>R : GET /api/stocks/{code}/analysis?refresh=true
R->>S : get_kline_data(force_refresh=True)
S-->>R : 强制刷新的K线数据
R-->>A : 最新分析数据
A-->>P : 返回最新数据
P->>P : 更新图表数据
...
```mermaid
flowchart TD
Start(["进入 AnalysisPage"]) --> Fetch["调用 getStockAnalysis 获取分析数据<br/>设置缓存控制头"]
...
Interact --> Refresh["用户点击刷新按钮<br/>handleKlineRefresh"]
Refresh --> ForceFetch["调用 getStockAnalysis(refresh=true)<br/>强制刷新K线数据"]
ForceFetch --> UpdateChart["更新图表数据并显示最新数据"]
UpdateChart --> End(["完成"])
```
...
### K线刷新按钮功能
- **新增** 刷新按钮：AnalysisPage 中新增强制刷新按钮，位于K线图表标题右侧，使用ReloadOutlined图标。
- **状态管理**：klineRefreshing状态控制按钮的加载状态，防止重复刷新。
- **刷新逻辑**：handleKlineRefresh函数调用getStockAnalysis时传入refresh=true参数。
- **通知反馈**：刷新成功时显示成功消息，若远程数据拉取失败则显示警告消息提示使用缓存数据。
- **防抖机制**：通过klineRefreshing状态防止在刷新过程中重复点击。

```mermaid
flowchart TD
UserClick["用户点击刷新按钮"] --> CheckState{"检查klineRefreshing状态"}
CheckState --> |状态正常| SetLoading["设置klineRefreshing=true"]
CheckState --> |正在刷新中| End["结束防止重复刷新"]
SetLoading --> CallAPI["调用getStockAnalysis(refresh=true)"]
CallAPI --> Success{"请求成功？"}
Success --> |是| CheckData["检查最后一条数据日期"]
CheckData --> Today{"是否为今天？"}
Today --> |是| ShowSuccess["显示成功消息"]
Today --> |否| ShowWarning["显示警告消息使用缓存数据"]
Success --> |否| ShowError["显示错误消息"]
ShowSuccess --> ResetState["设置klineRefreshing=false"]
ShowWarning --> ResetState
...
**图表来源**
- [AnalysisPage.tsx:108-125](file://frontend/src/pages/AnalysisPage.tsx#L108-L125)
- [AnalysisPage.tsx:607-617](file://frontend/src/pages/AnalysisPage.tsx#L607-L617)
...
- **新增** K线刷新：用户可通过刷新按钮强制获取最新数据，增强图表展示的实时性。
...
- **新增** 强制刷新机制：通过refresh参数实现强制数据刷新，平衡数据新鲜度与性能。
- **新增** 缓存控制：NoCacheMiddleware 确保每次请求都获取最新数据，避免浏览器缓存导致的数据陈旧问题。
...
- API 封装：api.ts 统一封装请求方法，便于测试与替换实现，**新增**支持refresh参数。
...
### 浏览器缓存控制机制
- **后端实现**：NoCacheMiddleware 中间件检测 /api/ 路径的请求，强制设置 Cache-Control: no-store, no-cache, must-revalidate, max-age=0 和 Pragma: no-cache 响应头。
- **前端实现**：axios 实例设置默认请求头 Cache-Control: no-cache 和 Pragma: no-cache，确保每次请求都绕过浏览器缓存。
- **数据新鲜度保障**：结合前端内存缓存和后端数据库缓存，确保图表显示最新数据，避免缓存导致的数据显示延迟。
- **性能考虑**：虽然禁用了缓存，但通过后端本地缓存和前端内存缓存机制，仍能保持良好的响应性能。

```mermaid
flowchart TD
Start(["请求图表数据"]) --> FrontHeader["前端 axios 设置缓存控制头"]
FrontHeader --> BackendCheck["后端 NoCacheMiddleware 检测 /api/ 路径"]
BackendCheck --> SetHeaders["设置 Cache-Control/Pragma 响应头"]
SetHeaders --> ProcessRequest["处理 API 请求"]
ProcessRequest --> ReturnData["返回最新数据"]
ReturnData --> End(["图表显示最新数据"])
```
...
BE --> AK["akshare"]
BE --> PD["pandas"]
BE --> NC["NoCacheMiddleware<br/>缓存控制中间件"]
```
...
- **新增** 强制刷新机制：通过refresh参数实现按需强制刷新，平衡数据新鲜度与性能。
- **新增** 缓存控制：NoCacheMiddleware 确保数据新鲜度，避免缓存陈旧问题。
...
- **新增** 刷新功能问题：检查handleKlineRefresh函数是否正确调用getStockAnalysis且传入refresh=true参数。
- **新增** 缓存问题：如果遇到数据不更新的情况，检查 NoCacheMiddleware 是否正确设置响应头，以及前端 axios 是否正确发送缓存控制请求头。
...
## 结论
该图表展示集成以 React + ECharts 为核心，配合后端 pandas_ta 技术指标计算与建议生成，实现了从数据获取、指标计算到可视化呈现的完整链路。当前已具备 K 线、均线与成交量的叠加显示及基础交互能力；**新增** K线刷新按钮功能增强了图表展示的实时性，用户可通过点击按钮强制获取最新数据；后续可在前端扩展 MACD/KDJ/布林带叠加与区域填充，在后端引入更丰富的指标与预测模型，并在前端引入虚拟滚动与移动端优化，以进一步提升性能与用户体验。**新增**的浏览器缓存控制机制确保了数据的新鲜度，避免了缓存导致的数据显示问题。
...
- **新增** 刷新功能：AnalysisPage 中的 handleKlineRefresh 函数和 klineRefreshing 状态管理。
- **新增** 缓存控制配置：前端 axios 默认设置缓存控制头，后端 NoCacheMiddleware 强制设置响应头。
