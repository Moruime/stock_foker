...
- 扩展前端页面以支持新的AI分析功能
- **更新** 新增宏观页面增强、情绪页面增强、数据源服务增强等核心功能，提升了AI代理分析系统的完整性和用户体验
...
**更新** 新版本引入了完整的AI代理分析系统，包括消息面情绪分析、板块联动分析、宏观环境感知和增强版买卖建议，同时建立了独立的数据源缓存基础设施，支持多种外部数据源的统一管理。新增的宏观页面和情绪页面增强了AI分析的完整性和用户体验。
...
- **增强的数据源服务**：支持hithink_news、announcements、industry_valuation、market_data、industry_finance、industry_peers、hithink_index、reports、basicinfo、business、shareholders、concept_boards、north_flow、market_overview、hithink_macro、hithink_events等数据源
...
- **增强** 新增事件数据查询（hithink_events），提供更全面的业绩预告和重要事件分析
...
- **增强** 新增宏观经济指标展示，包括CPI、PPI、PMI、LPR、M2等关键指标
...
- **新增** north_flow：北向资金流向
- **新增** market_overview：市场涨跌概况
- **新增** hithink_macro：宏观经济指标
- **新增** hithink_events：公司重要事件
...
#### 支持的数据源类型
- **消息面数据**：hithink_news、announcements、reports、hithink_events
- **公司基本面**：basicinfo、business、shareholders
- **市场数据**：hithink_index、market_data、north_flow、market_overview
- **行业数据**：industry_valuation、industry_finance、industry_peers
- **宏观经济**：hithink_macro

#### 并行数据获取
- 使用ThreadPoolExecutor并行执行多个数据源请求
- 每个数据源独立try-catch，避免相互影响
- 支持最多8个并发任务，提高整体响应速度

```mermaid
flowchart TD
Start(["数据源请求"]) --> ParallelFetch["并行获取多个数据源"]
ParallelFetch --> Task1["获取hithink_news"]
ParallelFetch --> Task2["获取announcements"]
ParallelFetch --> Task3["获取reports"]
ParallelFetch --> Task4["获取hithink_events"]
Task1 --> MergeResults["合并结果"]
Task2 --> MergeResults
Task3 --> MergeResults
Task4 --> MergeResults
MergeResults --> CacheCheck{"缓存是否新鲜？"}
...
**图表来源**
- [backend/app/services/data_source_service.py:106-125](file://backend/app/services/data_source_service.py#L106-L125)
- [backend/app/services/data_fetcher.py:106-125](file://backend/app/services/data_fetcher.py#L106-L125)

**章节来源**
- [backend/app/services/data_source_service.py:1-169](file://backend/app/services/data_source_service.py#L1-L169)
- [backend/app/services/data_fetcher.py:1-200](file://backend/app/services/data_fetcher.py#L1-L200)

### 增强的前端数据源Hook

**更新** 新增独立的数据源Hook，提供更好的用户体验。

#### Hook功能特性
- 模块级内存缓存，跨组件共享且不触发重渲染
- 缓存新鲜度与Agent缓存边界同步（每天09:00）
- 支持强制刷新和自动缓存检查
- 内存缓存上限管理，避免内存泄漏

#### 缓存管理策略
- 内存缓存键值格式：`{stockCode}:ds:{sourceType}`
- 最大缓存条目：240条（约20只股票×12种数据源）
- 自动淘汰最旧条目，保持内存使用在合理范围
- 缓存过期检查：超过09:00边界自动失效

#### 数据源Hook集成
- 每个数据面板独立管理自己的加载/缓存/刷新状态
- 支持数据源级别的刷新操作
- 提供loading、error、timestamp、fromCache等状态信息

```mermaid
flowchart TD
Start(["useDataSource Hook"]) --> CheckStock{"是否有股票代码？"}
CheckStock --> |否| ReturnNull["返回空状态"]
CheckStock --> |是| MemCache["检查内存缓存"]
MemCache --> MemHit{"内存缓存命中？"}
MemHit --> |是| ReturnMem["返回内存缓存数据"]
MemHit --> |否| CheckForce{"强制刷新？"}
CheckForce --> |是| FetchAPI["调用API获取数据"]
CheckForce --> |否| CheckDB{"检查数据库缓存"}
CheckDB --> |是| ReturnDB["返回数据库缓存"]
CheckDB --> |否| FetchAPI
FetchAPI --> SaveCache["保存到内存缓存"]
SaveCache --> ReturnFresh["返回新数据"]
ReturnMem --> End(["结束"])
ReturnDB --> End
ReturnFresh --> End
...
- [frontend/src/hooks/useDataSource.ts:48-69](file://frontend/src/hooks/useDataSource.ts#L48-L69)
...
- **数据源Hook缓存问题**
  - 现象：数据源页面显示缓存错误或刷新无效
  - 排查：检查内存缓存是否过期；确认缓存键值格式正确；查看缓存清理逻辑
  - 参考路径：[frontend/src/hooks/useDataSource.ts:48-69](file://frontend/src/hooks/useDataSource.ts#L48-L69)
...
## 结论
Stock Foker 通过清晰的模块划分与稳健的技术选型，实现了从数据获取、指标计算到建议生成与画像分析的完整闭环。新版本引入的AI代理分析系统进一步提升了分析能力，通过消息面、板块、宏观等多维度的智能分析，为投资决策提供更全面的支持。独立的数据源缓存基础设施确保了系统的稳定性和性能，而位置管理功能则完善了投资管理的全生命周期。

**更新** 新版本的宏观页面增强、情绪页面增强和数据源服务增强显著提升了AI代理分析系统的完整性和用户体验。新增的独立数据源缓存服务支持多种外部数据源的统一管理，提高了数据获取的效率和可靠性。增强的数据源Hook提供了更好的内存缓存和刷新机制，改善了前端页面的响应速度和用户体验。

建议在后续版本中继续优化AI分析的准确性，扩展更多数据源类型，并增强个性化推荐能力。同时可以考虑增加更多的可视化图表和交互功能，进一步提升用户的分析体验。
...
- 功能使用示例与最佳实践
  ...
  - **位置管理最佳实践**
    ...
    - 定期评估持仓表现，及时调整投资组合
  - **新增功能使用建议**
    - **宏观页面**：关注宏观经济指标的变化趋势，结合市场阶段制定交易策略
    - **情绪页面**：利用AI提取的关键新闻和事件信息，及时把握市场情绪变化
    - **数据源Hook**：利用内存缓存机制，提升页面响应速度和用户体验
    - **并行数据获取**：充分利用并行处理能力，提高多数据源的获取效率

**章节来源**
- [doc/产品设计文档.md:20-288](file://doc/产品设计文档.md#L20-L288)
- [doc/技术架构文档.md:120-197](file://doc/技术架构文档.md#L120-L197)
- [frontend/src/pages/MacroPage.tsx:1-509](file://frontend/src/pages/MacroPage.tsx#L1-L509)
- [frontend/src/pages/SectorPage.tsx:1-468](file://frontend/src/pages/SectorPage.tsx#L1-L468)
- [frontend/src/pages/SentimentPage.tsx:1-575](file://frontend/src/pages/SentimentPage.tsx#L1-L575)
- [frontend/src/hooks/useDataSource.ts:1-169](file://frontend/src/hooks/useDataSource.ts#L1-L169)
- [backend/app/services/data_fetcher.py:106-125](file://backend/app/services/data_fetcher.py#L106-L125)