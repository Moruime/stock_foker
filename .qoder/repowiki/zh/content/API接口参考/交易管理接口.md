...
- [doc/代码审查/2026-04-14-phase2-trades-batch-review.md](file://doc/代码审查/2026-04-14-phase2-trades-batch-review.md)
</cite>

## 更新摘要
**变更内容**
- 新增批量删除交易记录API端点：POST /api/trades/batch-delete
- 更新前端交易页面，增加批量删除功能
- 新增批量删除结果返回结构定义
- 更新错误处理策略，包含批量删除的特定错误场景
...
- **批量删除**：POST /api/trades/batch-delete
...
- 交易记录路由集中于 stock_router.py，提供 GET/POST/PUT/DELETE 四个端点，**新增批量删除端点**。
...
#### **批量删除交易记录** **新增**
- 方法与路径：POST /api/trades/batch-delete
- 请求体：ids（数字数组）
- 响应：BatchDeleteResult
  - deleted：成功删除的记录数量
  - realtime_adjusted：实时交易记录中已调整持仓的数量
  - total：请求中提供的总记录数量
- 错误处理：
  - 当 ids 为空时返回 400
  - 当记录不存在时返回 404
- 示例（前端调用）：batchDeleteTradeRecords([1, 2, 3])

**章节来源**
- [backend/app/routers/stock_router.py:408-452](file://backend/app/routers/stock_router.py#L408-L452)
- [frontend/src/services/api.ts:70-77](file://frontend/src/services/api.ts#L70-L77)
...
- 400 参数错误：当批量删除时 ids 为空，返回 400 并提示"请选择要删除的记录"。
...
- [backend/app/routers/stock_router.py:414-415](file://backend/app/routers/stock_router.py#L414-L415)
...
- **批量操作优化**：批量删除通过单次数据库查询获取所有记录，然后统一提交，减少数据库往返次数。

## 故障排除指南
- 404 未找到：确认 trade_id 是否正确，或检查是否存在 stock_code 过滤导致无结果。
- 400 参数错误：检查批量删除时传入的 ids 数组是否为空或包含无效 ID。
...
## 结论
交易记录管理 API 提供了完整的 CRUD 能力，结合 stock_code 过滤、limit 限制与按 traded_at 降序排序，满足日常交易记录的查询与维护需求。**新增的批量删除功能**进一步提升了用户体验，支持一次性删除多个交易记录并自动处理实时交易的持仓调整。通过 Pydantic Schema 保证数据一致性，配合 SQLAlchemy ORM 实现稳定的数据访问。前端通过 Axios 封装的 API 方法与后端交互，形成清晰的前后端协作模式。
...
- **POST /api/trades/batch-delete**：**批量删除交易记录**
...
### 批量删除功能特性
- **智能持仓调整**：对于实时交易记录，删除时会自动反向调整对应的持仓信息
- **批量处理**：支持一次删除多个记录，提高操作效率
- **详细反馈**：返回删除统计信息，包括成功删除数量和已调整的实时交易数量
- **安全验证**：对空数组进行验证，防止误操作

**章节来源**
- [doc/代码审查/2026-04-14-phase2-trades-batch-review.md:38-50](file://doc/代码审查/2026-04-14-phase2-trades-batch-review.md#L38-L50)
- [frontend/src/pages/TradesPage.tsx:176-211](file://frontend/src/pages/TradesPage.tsx#L176-L211)