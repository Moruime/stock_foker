...
## 更新摘要
**变更内容**
- 新增 DataSourceCache 模型支持，启用数据源缓存的数据库持久化存储
- 添加 proper indexing for efficient querying 的索引策略
- 更新数据源缓存的查询和写入机制
- 扩展缓存新鲜度边界（每天 09:00）的实现
- 增强数据源缓存的降级策略和错误处理
...
## 简介
本文件系统化梳理 Stock Foker 应用的数据库表结构，重点覆盖以下四张表：
- FocusStock：当前关注的股票
- TradeRecord：交易操作记录
- KlineCache：K线数据本地缓存
- DataSourceCache：数据源缓存（新增）
...
A --> G
H["routers/data_source_router.py<br/>数据源路由"] --> I["services/data_source_service.py<br/>数据源服务"]
I --> F
J["routers/agent_router.py<br/>代理路由"] --> F
```
...
- [models.py:1-196](file://backend/app/models/models.py#L1-L196)
- [database.py:1-34](file://backend/app/db/database.py#L1-L34)
- [data_source_router.py:1-43](file://backend/app/routers/data_source_router.py#L1-L43)
- [data_source_service.py:1-273](file://backend/app/services/data_source_service.py#L1-L273)
- [agent_router.py:540-555](file://backend/app/routers/agent_router.py#L540-L555)
...
- DataSourceCache（数据源缓存）
  - 用途：独立于 Agent 结果，存储原始 hithink API 响应数据
  - 关键字段：stock_code、source_type、cache_key、data、created_at
  - 主键：id（自增整型）
  - 唯一约束：stock_code + source_type + cache_key（确保每只股票每种数据源每天的唯一性）
  - 索引策略：stock_code、source_type 字段已建立索引
...
participant DSRouter as "data_source_router"
participant DSService as "data_source_service"
participant Model as "models(models.py)"
participant DB as "database.py"
App->>App : "on_startup 事件"
App->>DB : "init_db()"
DB->>DB : "Base.metadata.create_all(bind=engine)"
Note over DB : "创建 FocusStock / TradeRecord / KlineCache / DataSourceCache 表"
App-->>Router : "注册路由"
App-->>DSRouter : "注册数据源路由"
Router->>Service : "调用业务方法"
DSRouter->>DSService : "调用数据源服务"
Service->>Model : "ORM 查询/写入"
DSService->>Model : "ORM 查询/写入"
Model->>DB : "通过 SessionLocal 执行 SQL"
```

**图表来源**
- [main.py:20-23](file://backend/app/main.py#L20-L23)
- [database.py:32-34](file://backend/app/db/database.py#L32-L34)
- [stock_router.py:1-197](file://backend/app/routers/stock_router.py#L1-L197)
- [data_source_router.py:1-43](file://backend/app/routers/data_source_router.py#L1-L43)
- [stock_service.py:131-151](file://backend/app/services/stock_service.py#L131-L151)
- [data_source_service.py:168-221](file://backend/app/services/data_source_service.py#L168-L221)
- [models.py:25-131](file://backend/app/models/models.py#L25-L131)
...
**图表来源**
- [models.py:25-41](file://backend/app/models/models.py#L25-L41)
...
### DataSourceCache 表（新增）
- 表名：data_source_cache
- 主键：id（整型，自增）
- 唯一约束
  - uq_ds_cache：stock_code + source_type + cache_key 唯一
- 字段与约束
  - id：整型，主键，自增
  - stock_code：字符串，长度限制，非空，已建立索引
  - source_type：字符串，长度限制，非空，已建立索引
  - cache_key：字符串，长度限制，非空（日期 "YYYY-MM-DD"）
  - data：文本，JSON 格式，非空
  - created_at：时间戳
- 索引策略
  - 已在 stock_code 和 source_type 上建立索引，满足常见的查询模式
  - cache_key 字段用于按日期查询，建议在高并发场景下考虑添加复合索引
- 业务含义与取值范围
  - stock_code：股票代码
  - source_type：数据源类型（如 hithink_news、announcements、reports 等）
  - cache_key：缓存键，使用日期格式 "YYYY-MM-DD"
  - data：原始 API 响应数据的 JSON 序列化
  - created_at：缓存创建时间
- 缓存策略
  - 缓存新鲜度边界：每天 09:00，超过该时间的缓存被视为新的日期
  - 降级策略：当 API 返回空数据时，回退到最近的有效历史缓存
  - 并发安全：支持多线程并行获取多个数据源
- DDL（示意）
  - 创建表时需包含主键、唯一约束与索引
- 外键关系
...
DATA_SOURCE_CACHE ||--o{ "唯一约束(uq_ds_cache)" : "stock_code+source_type+cache_key"
```
...
**章节来源**
- [models.py:118-131](file://backend/app/models/models.py#L118-L131)
- [data_source_service.py:93-148](file://backend/app/services/data_source_service.py#L93-L148)
- [data_source_router.py:22-43](file://backend/app/routers/data_source_router.py#L22-L43)

## 依赖分析
- 数据模型依赖
  - models.py 定义了四个表与三个枚举（TimeFrame、TradeType、MarketSentiment）
  - database.py 提供 Base 类、引擎与会话工厂，并暴露 init_db 方法
  - main.py 在应用启动时调用 init_db
  - routers 与 services 通过 SQLAlchemy ORM 与数据库交互
  - data_source_service.py 专门处理 DataSourceCache 的查询与写入
  - agent_router.py 包含清理缓存的功能，同时清理 DataSourceCache
- 外部依赖
  - requirements.txt 明确了 SQLAlchemy 版本要求
  - stock_service 使用 akshare、pandas、pandas-ta 等库进行数据获取与指标计算
  - data_source_service 使用多种数据源 API（hithink API、AKShare 等）

```mermaid
graph LR
M["models.py"] --> D["database.py"]
D --> S["stock_service.py"]
D --> DS["data_source_service.py"]
S --> R["stock_router.py"]
DS --> DR["data_source_router.py"]
R --> A["main.py"]
DR --> A
AR["agent_router.py"] --> M
...
- 索引优化建议
  ...
  - DataSourceCache：已在 stock_code 和 source_type 上建立索引，建议在高并发场景下考虑添加 (stock_code, source_type, cache_key) 的复合索引以优化缓存查询性能
- 查询性能考量
  - DataSourceCache 的缓存策略先查本地再拉取远程，减少网络开销
  - KlineCache 的唯一约束保证了同一周期同一天的唯一性，有利于增量写入与去重
  - stock_service 的缓存策略先查本地再拉取远程，减少网络开销
  - 对于高频查询的字段（如 stock_code、traded_at、date、source_type），建议结合业务场景评估是否增加索引
- 写入性能考量
  - DataSourceCache 支持并行获取多个数据源，使用线程池优化性能
  - KlineCache 的批量写入与按日期增量更新，有助于降低重复写入成本
  - TradeRecord 的实际结果补充更新采用按需更新字段的方式，减少不必要的写放大
- 缓存策略优化
  - DataSourceCache 实现了智能缓存策略，包括新鲜度边界、降级回退、错误处理等
  - 前端实现了模块级内存缓存，与后端数据库缓存形成双重保护

**章节来源**
- [data_source_service.py:232-273](file://backend/app/services/data_source_service.py#L232-L273)
- [useDataSource.ts:1-168](file://frontend/src/hooks/useDataSource.ts#L1-L168)
...
- 表未创建
  ...
  - 检查数据库连接 URL 是否正确
- 数据重复
  - DataSourceCache 的唯一约束应防止重复插入；若仍出现重复，检查唯一键组合与日期格式
  - KlineCache 的唯一约束应防止重复插入；若仍出现重复，检查唯一键组合与日期格式
- 查询缓慢
  - 为高频过滤与排序字段添加索引
  - 检查是否存在全表扫描的查询
  - DataSourceCache 建议添加复合索引以提升查询性能
- 数据异常
  - 关注 created_at/updated_at 默认值行为与数据库时区设置
  - 检查枚举字段取值是否符合定义范围
  - DataSourceCache 的 JSON 数据格式是否正确
- 缓存问题
  - 检查缓存新鲜度边界（每天 09:00）是否正确
  - 确认降级策略是否正常工作
  - 验证并行获取数据源时的线程安全
...
## 结论
本文档系统化梳理了 Stock Foker 的四张核心表：FocusStock、TradeRecord、KlineCache、DataSourceCache。明确了字段定义、数据类型、约束与索引策略，解释了主键/唯一/外键设计与业务含义，并给出了数据库初始化流程、表创建顺序、索引优化建议与查询性能考量。

**更新内容**：新增了 DataSourceCache 模型的支持，这是一个重要的数据源缓存表，用于存储原始的 hithink API 响应数据。该表具有完善的索引策略和缓存机制，支持每天 09:00 的新鲜度边界、降级回退策略和并行获取功能。

建议在生产环境中根据实际查询模式进一步细化索引策略，特别是为 DataSourceCache 添加复合索引以提升缓存查询性能，并持续监控表结构与查询性能。
...
- DataSourceCache（新增）
  - 主键：id
  - 非空：stock_code、source_type、cache_key、data、created_at
  - 唯一约束：stock_code + source_type + cache_key
  - 建议索引：stock_code、source_type；高并发场景建议添加 (stock_code, source_type, cache_key) 复合索引

**章节来源**
- [models.py:25-131](file://backend/app/models/models.py#L25-L131)

### 缓存策略说明
- DataSourceCache 实现了智能缓存策略：
  - 缓存新鲜度边界：每天 09:00，超过该时间的缓存被视为新的日期
  - 降级回退：当 API 返回空数据时，回退到最近的有效历史缓存
  - 并发安全：支持多线程并行获取多个数据源
  - 错误处理：写入失败时记录警告信息并回滚事务
  - 前端内存缓存：与后端数据库缓存形成双重保护

**章节来源**
- [data_source_service.py:79-148](file://backend/app/services/data_source_service.py#L79-L148)
- [useDataSource.ts:1-168](file://frontend/src/hooks/useDataSource.ts#L1-L168)