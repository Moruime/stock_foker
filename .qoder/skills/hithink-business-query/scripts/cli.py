...
class BusinessDataAPIError(Exception):
    """API错误异常类"""
...
def query_business_data(query: str, page: str, limit: str,
    ...
    Raises:
        BusinessDataAPIError: API调用失败时抛出
    """
    # 获取API密钥
    if not api_key:
        api_key = os.environ.get("IWENCAI_API_KEY", "")
    if not api_key:
        raise BusinessDataAPIError("API密钥未设置，请通过参数或环境变量IWENCAI_API_KEY指定")
    ...
    try:
        ...
        with urllib.request.urlopen(request, timeout=30, context=_ssl_ctx) as response:
            ...
            if isinstance(result, dict):
                ...
                if status_code != 0:
                    ...
                    raise BusinessDataAPIError(f"API返回错误: {status_msg}")
        ...
        raise BusinessDataAPIError(
            f"HTTP错误 {e.code}: {e.reason}",
            status_code=e.code,
            response=error_body
        )
    except urllib.error.URLError as e:
        raise BusinessDataAPIError(f"网络错误: {e.reason}")
    except json.JSONDecodeError as e:
        raise BusinessDataAPIError(f"响应解析失败: {e}")
...
def main():
    ...
    except BusinessDataAPIError as e:
        error_output = {
            "success": False,
            "error": e.message
        }
